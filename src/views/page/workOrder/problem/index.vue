...
<!--    <div class="problem-header">我的问题</div>-->
    <van-list v-model="loading" :finished="finished" :immediate-check="false" loading-text="加载中..." finished-text="没有更多了" @load="getProblemData">
      <ul class="problem-list">
        <li  v-for="(item,index) in problemData" :data="item.id" @click="jumpRoute('/questionDetails',{workorder_id:item.id})">
          ...
          <van-steps :active="item.state"  active-color="#38f" style="margin-top: 20px;padding-left: 0;padding-right: 0;">
  ...
  <!--        <div class="problem-problem"><span>{{ item.problem_type }}</span></div>-->
          <div class="problem-list-right">
            ...
            <p><span>进度:</span>
              ...
              <span v-if="item.state===3">已结束</span></p>
          </div>
        </li>
      </ul>
    </van-list>
